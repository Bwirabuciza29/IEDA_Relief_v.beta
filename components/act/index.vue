<template>
  <div class="mj-container bg-white mt-8">
    <div class="relative w-full bg-center bg-cover flex items-center">
      <div class="us-container my-4">
        ...
        <div class="border-b mb-5 mt-8 flex justify-between text-sm">
          <div
            class="text-primary_1 flex items-center pb-2 pr-2 border-b-4 border-custom-green uppercase"
          >
            <svg
              class="h-6 mr-3"
              version="1.1"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-10 mt-4">
          ...
          >
            ...
            <main class="p-5 z-10">
              <a
                href="#"
                class="text-md tracking-tight font-medium leading-7 font-regular text-white"
              >
                {{ article.titre }}
              </a>
            </main>
...
const articlesFiltres = computed(() => {
  if (!props.pays) return [];

  return articles.value.filter((article) => {
    const designation = article.pays?.designation?.toLowerCase?.();
    return designation === props.pays.toLowerCase();
  });
});

const articlesTries = computed(() =>
  [...articlesFiltres.value].sort(
